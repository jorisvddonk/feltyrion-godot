...
void tree(float x, float y, float z, int32_t depth) {
    ...
    if (treetype == GIANT_TREE) {
        ...
        if (depth > 07) {
            build_fractal_tree(x, y, z, 2 * treescaling, treespreads, 1.5 * branchwidth, 3, 2, 1.5 * treepeaking,
                               rootshade, 0xC0, 120, 1, 0);
            return;
        }

        if (depth > 04) {
            build_fractal_tree(x, y, z, 2 * treescaling, treespreads, 1.5 * branchwidth, 4, 3, 1.5 * treepeaking,
                               rootshade, 0xC0, 120, 1, 0);
            return;
        }

        build_fractal_tree(x, y, z, 2 * treescaling, treespreads, 1.5 * branchwidth, 4, 3, 1.5 * treepeaking, rootshade,
                           0xC0, 72, 1, 0);
        return;
    }
    ...
    if (treetype == FORCE_LAT) {
        build_fractal_tree(x, y, z, treescaling, treespreads, branchwidth, 2, 3, 2 * treepeaking, 0x80, 0xC0, 120, 1,
                           0);
        return;
    }

    if (treetype == FORCE_CON) {
        build_fractal_tree(x, y, z, treescaling, treespreads, branchwidth, 2, 2, 0.5 * treepeaking, 0x80, 0x40, 120, 1,
                           0);
        return;
    }

    build_fractal_tree(x, y, z, treescaling, treespreads, branchwidth, 2, 2, treepeaking, rootshade, 0xC0, 120, 1, 0);
}
...
struct pvlist mamm_ears[5] = {
    {42, 0, 1, 0, 0}, {45, 0, 0, 1, 0}, {43, 1, 0, 0, 0}, {44, 0, 0, 1, 0}, {0xFFF, 0, 0, 0, 0}};

const int16_t mamm_wrap_center_p = 16;
const int16_t mamm_wrap_center_v = 2;

struct pvlist mamm_reartoto[19] = {{7, 0, 0, 1, 1},  {8, 1, 1, 1, 1},  {9, 1, 1, 1, 1},    {14, 1, 1, 1, 1},
...
struct pvlist mamm_legs[15] = {{0, 1, 1, 1, 1},                                                        // F-L
...
struct pvlist mamm_tail[7] = {{46, 1, 1, 1, 1}, {47, 1, 1, 1, 1}, {50, 1, 1, 1, 1},   {51, 1, 1, 1, 1},
                              {48, 1, 1, 1, 0}, {49, 1, 1, 1, 0}, {0xFFF, 0, 0, 0, 0}};
...
inactive:
    ...
    if (ani_type[n] == BIRD) {
        ...
        if (ay < 0 || sctype != OCEAN) {
            if (ani_quote[n] < 50) {
                ...
                modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, +45 * dy, +75 * dy,
                      bird_wing1);
                modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, -45 * dy, -75 * dy,
                      bird_wing2);
                goto bird_trace;
        ...
        if (ani_scale[n] > 10) {
            ...
            modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, 0, fabs(10 - (tick % 20)) * -4.5,
                  bird_wing1);
            modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, 0, fabs(10 - (tick % 20)) * +4.5,
                  bird_wing2);
        } else {
            ...
            modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, 0, fabs(3 - (tick % 6)) * -15,
                  bird_wing1);
            modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, 0, fabs(3 - (tick % 6)) * +15,
                  bird_wing2);
        }
    ...
    // visualizzazione:
    bird_trace:
        modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, dz, 0, 0, 0);
        modpv(bird_result, bird_wings_center_p, bird_wings_center_v, 1, 1, 1, 0, ani_pitch[n], 0, 0);
        drawpv(bird_result, texture_skin_map, 3, ax, ay - 9 * ani_scale[n], az, perform_depth_sort);
    ...
    if (ani_type[n] == MAMMAL) {
        ...
        if (ay > -10 && sctype == OCEAN) {
            ...
            modpv(mamm_result, mamm_wrap_center_p, mamm_wrap_center_v, 1, 1, 1, incl, 0, 0, NULL);
            ...
            if (ani_speed[n] < 50) {
                ...
                if (fast_random(1)) {
                    period = fabs(fsecs - 0.5);
                    modpv(mamm_result, mamm_tail_center_p, mamm_tail_center_v, 1, 1, 1, 0, 240 * period - 60, 0,
                          mamm_tail);
                }
            } else {
                ...
                modpv(mamm_result, mamm_wrap_center_p, mamm_wrap_center_v, 1, 1, 1, -50 * period, 0, 0, NULL);
                modpv(mamm_result, mamm_wrap_center_p, mamm_wrap_center_v, 1, 1, 1, 100 * period, 0, 0, mamm_reartoto);
            }
        }

        modpv(mamm_result, mamm_wrap_center_p, mamm_wrap_center_v, 1, 1, 1, 0, ani_pitch[n] + 180, 0, NULL);
    // visualizzazione:
    mamm_trace:
...
void fragmentWC(int32_t x, int32_t z, int8_t captureWhat) {
    ...
    // Surface shading component at this point
    if (sh_delta) {
        // Incident light
        c1 = p_surfacemap[h1] - p_surfacemap[h1 + sh_delta];
    } else {
        // Almost perpendicular to the sun? Diffused light.
    ...
    if (!mirror) {
        ...
        if (poly1 || poly2) {
            ...
            // Set the color of the grassy outgrowths.
            if (groundflares == 8) {
                if (depth >= 16) {
                    flares = 0;
                } else {
        ...
        // Trace the capsule in which you descened. It emits a bright
        // beam, to make  it easy to find, given that the terrain is
        // quiute vast (you can go up to 3km from your point of
        // disembarkation).
...
void round_hill(int16_t cx, int16_t cz, uint16_t r, float h, float hmax, int8_t allowcanyons) {
...
void std_crater(uint8_t *map, int16_t cx, int16_t cz, int16_t r, int16_t lim_h, float h_factor, float h_raiser,
                int32_t align) {
    // Un cratere.
    int16_t x, z;
...
void srf_darkline(uint8_t *map, int16_t length, int16_t x_trend, int16_t z_trend, int32_t align) {
...
void felisian_srf_darkline(uint8_t *map, int16_t length, int16_t x_trend, int16_t z_trend, int32_t align) {
...
void asterism(uint8_t *map, int16_t x, int16_t y, int16_t base, int16_t variation, int16_t density, int16_t size,
              int32_t align) {
...
void make_ruins(int8_t style1, int8_t style2, int8_t style3, int8_t style4, int8_t style5, int16_t density) {
    int16_t buildings = 0;
...
void build_surface() {
    ...
    case 5: // con atmosfera sottile (marte etc...)
        ...
        if (albedo > 40 && albedo <= 50) {
            ...
            std_crater(p_surfacemap, 90 + fr2, 90 + fr1, 100 + fr0, 127, hf, hr, 200);
        }
...
void setup_animals() {
    ...
    fast_srand(global_surface_seed);
    // mammiferi:
    loadpv(mamm_base, mammal_ncc, 1, 0.5 + 0.5 * flandom(), 0.75 + 0.5 * flandom(), 0, 0, 0, fast_random(0xC0), 1);
    ...
    // uccelli:
    loadpv(bird_base, birdy_ncc, 1, 0.3 + flandom(), 0.75 + flandom(), 0, 0, 0, fast_random(0xC0), 1);
...
void planetary_main() {
    ...
    if (nearstar_p_owner[ip_targetted] != -1 && nearstar_p_type[nearstar_p_owner[ip_targetted]] == 10) {
...
nosecondarysun:
    ...
    do {
        ...
        if (resolve <= 63) {
            tavola_colori((const uint8_t *) surface_palette, 0, 256, resolve, resolve, resolve);
            resolve += 4;
        }
        ...
        ptr -= 639;
        background(360 * (int16_t) (alfa + 51) - (int16_t) (beta) % 360, adapted, s_background, n_offsets_map, om_bytes,
                   ptr);
        ...
        if (nearstar_p_type[ip_targetted] == 3) {
            ...
            if (sctype == PLAINS || sctype == DESERT) {
                ...
                // luccich?i mirati (in riflesso diretto della luce della
                // stella)
                if (!nightzone) {
        ...
        if (pos_y > crcy) {
            ...
            if (!landed) {
                ...
                if (!opencapdelta && gravity < 250 && (compdist < 512 || bounces > 10)) {
        ...
        if (nearstar_p_type[ip_targetted] == 3) {
            if (rainy >= 2 || flashes > 5) {
                ...
                if (rainy > 3) {
                    ...
                    while (ptr) {
                        ...
                        stick3d(drop_x, drop_y, drop_z, drop_x + wdircos, drop_y + 100, drop_z + wdirsin);
                        ptr--;
                    }
            ...
            if (pos_y >= 0) {
                ...
                if (nearstar_p_type[ip_targetted] == 3 && (sctype == PLAINS || sctype == DESERT)) {
